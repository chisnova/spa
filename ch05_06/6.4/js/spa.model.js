...
spa.model = (function () {
    ...
    //    Person 객체를 반환한다. 채팅 상대가 없다면 null을 반환한다.
    //  * set_chatee( <person_id> ) - 채팅 상대를 
    //    person_id로 식별한 사람으로 설정한다.
    //    person_id가 사람 목록에 존재하지 않으면 채팅 상대(chatee)는 null로 설정된다.
    //    요청한 채팅 상대와 이미 채팅 중이면 false를 반환한다.
    //    이 메서드는 ‘spa-setchatee’ 전역 커스텀 이벤트를 발송한다.
    //  * send_msg( <msg_text> ) - 채팅 상대에게 메시지를 전송한다.
    ...
    //    백엔드로 전송한다. 
    //    이렇게 하면 'spa-listchange' 이벤트가 일어나고,
    //    업데이트된 사람 목록 및 아바타 정보(Person 객체에 있는 css_map)가 전달된다
    //    update_avtr_map은 반드시
    //    { person_id : person_id, css_map : css_map } 형식이어야 한다.
    //
    // chat 객체에서 발송하는 전역 커스텀 이벤트는 다음과 같다.
    //  * spa-setchatee - 새 채팅 상대를 설정할 때 
    //    발송된다. 
