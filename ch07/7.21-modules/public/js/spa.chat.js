...
spa.chat = (function () {
	...
	var
    	configMap = {
			...
			slider_opened_em 	: 18,
			slider_closed_em 	: 2,
			slider_opened_title : 'Tab to close',
			slider_closed_title : 'Tab to open',
	...
	//    false를 반환해야 한다.
	// 	* chat_model - 채팅 모델 객체는 인스턴트 메시징과 상호작용하는
	// 	  	메서드를 제공
	// 	* people_model - people 모델 객체는 모델에서 가진 사람들 목록을 관리하기 위한 
	// 	  	메서드를 제공
	// 	* slider_* settings. 이들 설정은 모두 선택 스칼라값이다.
	// 	  	전체 설정 목록은 mapConfig.settable_map을 참고.
	// 		예시: slider_open_em은 em으로 지정한 열린 높이다.
	// 행동 	:
	// 	넘겨받은 인자를 사용해 내부 설정 데이터 객체(configMap)를 업데이트한다.
	// 	그 외 다른 행동은 수행하지 않는다.
	// 반환값	: true
	// 예외	: 자바스크립트 에러 객체 및 넘겨받을 수 없거나 빠져 있는 인자에 대한
	// 		  스택트레이스
	//
